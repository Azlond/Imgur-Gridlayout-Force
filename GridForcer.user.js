...
if (!regex.test(currentLink)) { // if there is no "grid" in the current url
	...
	if (res[res.length - 1].length < 1) { //if there's a "/" as last character, we need to get the second to last element from the array
		galleryID= res[res.length - 2]; // get the second to last element of the array, which is the ID of the gallery
